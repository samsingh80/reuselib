...
sap.ui.controller("com.kaust.zui5approvers.controller.GovtVistVisaExten", {
	onInit: function () {
		...
		this.getView().byId("gvv_requestid").setText(requestId);
		...
		if (data != null) {
			...
			odlrModel.read("GASC_HeaderSet?$filter=RequestId eq'" + requestId + "'&$expand=Headertoiqama", null, null, false, function (data,
					...
					var oDetailsModel = new sap.ui.model.json.JSONModel();
					 that.GvvRequestid = data.results[0].RequestId;
				...
				var oDetailsModel = new sap.ui.model.json.JSONModel();
				oGASCModel.read("UserDetail(KaustID='"+FEVHead.RequestorKaustid+"',UserId='')", null, null, false, function(oData, response) {
					oDetailsModel.setData(oData);
					that.getView().setModel(oDetailsModel,'Details');
				});
			
// Populate Attachment Details - Start
			var doctype;
			var empnation;
			var GVVAttJson1;
			var GVVEXTAttJson;
			...
			var attxt = "FileRead?$filter=UNIQUE_ID eq '" + that.GvvRequestid +
			...
			if (GVVEXTAttJson.results.length > 0) {
				for (var i = 0; i < GVVEXTAttJson.results.length; i++) {
					...
					if (split[0] === "NOL") {
						this.getView().byId("GVPassport").setHref(GVVEXTAttJson.results[i].URL);
					} else {
						this.getView().byId("GVSaudiB").setHref(GVVEXTAttJson.results[i].URL);
					}
				}
			}
